...
const SearchContent = ({ data }) => {
    ...
    return (
        <View className="flex-1">
            <ScrollView
                className="mt-3 h-[calc(100vh+12%)]"
                showsVerticalScrollIndicator={false}
            >
                {individual.filter(handleFilter).map((room) => (
                    <Pressable
                        onPress={() => handleNevigate(room)}
                        key={room.roomname}
                        className="flex-row justify-start rounded-[12px] bg-[#e6eff6] py-2  px-2 items-center mb-2"
                        style={{ rowGap: 20 }}
                    >
                        <View className="w-[60px] h-[60px] rounded-full  bg-[#edf6fd] p-3 mr-3 object-cover">
                            <Image
                                source={boxIcon[room.title]}
